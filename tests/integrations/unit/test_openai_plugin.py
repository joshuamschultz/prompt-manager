...
class TestOpenAIPlugin:
    """Test suite for OpenAIPlugin class."""

    @pytest.fixture
    def plugin(self) -> OpenAIPlugin:
        """Create an OpenAIPlugin instance."""
        return OpenAIPlugin()
    ...
    def test_plugin_name(self, plugin: OpenAIPlugin) -> None:
        """Test that plugin has correct name."""
        assert plugin.name == "openai"

    def test_plugin_version(self, plugin: OpenAIPlugin) -> None:
    ...
    async def test_render_for_framework_chat_prompt(
        self, plugin: OpenAIPlugin, chat_prompt: Prompt
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_validate_compatibility_returns_true(
        self, plugin: OpenAIPlugin, text_prompt: Prompt
    ) -> None:
        """Test that OpenAI plugin is compatible with TEXT format."""
        # Arrange
        await plugin.initialize({})
    ...
    @pytest.mark.asyncio
    async def test_get_config_returns_copy(self, plugin: OpenAIPlugin) -> None:
    ...
    def test_repr(self, plugin: OpenAIPlugin) -> None:
        """Test string representation of plugin."""
        # Act
        result = repr(plugin)

        # Assert
        assert "OpenAIPlugin" in result
        assert "name='openai'" in result
    ...
    async def test_render_multiple_format_prompts(
        self, plugin: OpenAIPlugin, text_prompt: Prompt, chat_prompt: Prompt
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_reinitialization_allowed(self, plugin: OpenAIPlugin) -> None:
